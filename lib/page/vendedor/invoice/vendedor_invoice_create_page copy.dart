...
class _VendedorCreateInvoiceScreenState
  ...
  Future<String?> _saveInvoice() async {
    ...
    final totalPago = _getMontoRecibido();
    ...
    try {
      ...
      final caja = await CajaService.getCurrentCaja();
      debugPrint('Obteniendo información del negocio');
      ...
      if (_comprobanteFile != null) {
        final fileName =
            'facturas/${negocio!.nombre}/${_invoiceNumberController.text}/${DateTime.now().millisecondsSinceEpoch}';
        _logoKey = await StorageService.uploadFile(
          File(_comprobanteFile!.path),
          fileName,
        );
      }
      ...
      final invoice = Invoice(
        ...
        invoiceImages: _logoKey != null && _logoKey!.isNotEmpty
            ? [_logoKey!]
            : null,
        invoiceStatus: _selectedStatus,
        //invoicePayments: [_selectedPaymentType.name],
      ...
      List<InvoicePayment> pagosSeleccionados = _paymentOptions
          ...
          .toList();
      // Paso 3: guardar cada pago
      for (final pago in pagosSeleccionados) {
        final request = ModelMutations.create(pago);
      ...
      for (final itemData in _invoiceItems) {
        ...
        final int unidadesVendidas =
            itemData.quantity * itemData.precio!.quantity;
        final updatedProductRequest = itemData.producto.copyWith(
          stock: itemData.producto.stock - unidadesVendidas,
          isDeleted: itemData.producto.isDeleted,
          negocioID: itemData.producto.negocioID,
          barCode: itemData.producto.barCode,
          descripcion: itemData.producto.descripcion,
          categoriaID: itemData.producto.categoriaID,
          nombre: itemData.producto.nombre,
          precioCompra: itemData.producto.precioCompra,
          proveedorID: itemData.producto.proveedorID,
          productoImages: itemData.producto.productoImages,
          estado: itemData.producto.estado,
          createdAt: TemporalDateTime.now(),
          updatedAt: TemporalDateTime.now(),
        );

        debugPrint('Stock actualizado: $updatedProductRequest');

        final updateProductRequest = ModelMutations.update(
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Factura creada')));
    ...
    return Column(
      children: [
        ...
        ProductQuickSelector(
          ...
          onProductSelected: (invoiceItem, _) {
            setState(() {
    ...
    return Row(
      ...
      children: [
        ...
        Expanded(
          ...
          child: Row(
            children: [
              Expanded(child: _buildCantidadField(item, index)),
              const SizedBox(width: 12),
