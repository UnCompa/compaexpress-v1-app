...
class OrderDetailScreen extends StatelessWidget {
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        FutureBuilder<List<OrderPayment>>(
                          future: _fetchOrderPayments(),
                          builder: (context, snapshot) {
  ...
  Future<List<OrderPayment>> _fetchOrderPayments() async {
    final request = ModelQueries.list(
      OrderPayment.classType,
      where: OrderItem.ORDERID.eq(order.id),
    );
    final response = await Amplify.API.query(request: request).response;
    return response.data?.items.whereType<OrderPayment>().toList() ?? [];
  }
